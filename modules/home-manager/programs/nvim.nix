    ...
    extraLuaConfig = ''
      ...
      vim.api.nvim_create_autocmd("VimEnter", {
        callback = function()
          ...
          -- Open Minimap
          vim.cmd("Minimap")
          -- Return focus to the main editor window (optional)
          vim.cmd("wincmd l")
        end,
      })
       ...
       -- Minimap Toggle (<leader>m)
       vim.keymap.set('n', '<leader>m', ':MinimapToggle<CR>', opts)

      -- Select All (Ctrl + a) - Preserves Cursor Position
       vim.keymap.set({'n', 'i', 'v'}, '<C-a>', function()
         local saved_view = vim.fn.winsaveview()
         -- gg=top, V=Visual Line, G=bottom
         vim.cmd("normal! ggVG") 
         -- Restore the scroll and cursor position
       ...
       -- Theme Swapper (<leader>th)
       vim.keymap.set('n', '<leader>th', function()
         local builtin = require('telescope.builtin')
         local themes = { "catppuccin", "tokyonight", "nightfox", "nordfox", "duskfox", "terafox" }
         ...
         builtin.colorscheme({
           ...
           attach_mappings = function(prompt_bufnr, map)
             ...
             -- Override default selection to also save to our cache file
             actions.select_default:replace(function()
               local selection = action_state.get_selected_entry()
               actions.close(prompt_bufnr)
               if selection then
                 vim.cmd.colorscheme(selection.value)
                 save_theme(selection.value) -- This calls your existing save function
               end
             end)
             return true
           end,
         })
       end, { desc = "Swap Themes" })
